...
import nari.QueryService.bean.RelationType;
...
public class GetAllSubSet {
	...
	public GetAllSubSetResponse getAllSubSet(
			GetAllSubSetRequest request) {
		GetAllSubSetResponse resp = new GetAllSubSetResponse();
		String pclassId = request.getPclassId();
		...
		// 得到所有站设备  
		try {
			model = ms.fromClass(pclassId, false);
		} catch (Exception e) {
			logger.info("模型创建时出错");
		...
		Expression exp = builder.equal(
				builder.getRoot().get("SBID", String.class), sbid);
		...
		//关系种类
		RelationType relationType = request.getRelationType();
		String sql = "";
		switch (relationType) {
		case TOPOLINK:
			sql = "select * from " + TableName.CONF_MODELRELATION + " where relationid = 1001 and pmodelid = '"+ pModelId + "'";
			break;
		case CONTAIN:
			sql = "select * from " + TableName.CONF_MODELRELATION + " where relationid = 3002 and pmodelid = '"+ pModelId + "'";
			break;
		case CONTAINED:
			sql = "select * from " + TableName.CONF_MODELRELATION + " where relationid = 3001 and pmodelid = '"+ pModelId + "'";
			break;
		}
		...
		//要显示的model对应集合()
				...
				Map<DeviceModel,String> modelMap = new HashMap<DeviceModel,String>();
				String subRelateField = "";
				for (int i = 0; i < list.size(); i++) {
					...
					subRelateField = String.valueOf(list.get(i).get("relationfield"));
					try {
						...
						if (modelMap.containsKey(Model) || (!modelDisplayFlag)) { 	//相同或者不需要显示
							continue;
						}
						modelMap.put(Model,subRelateField);
					} catch (Exception e) {
		...
		// 得到该站内设备的类型model集合，根据对应的站(根据subRelateField)得到具体的设备
		Iterator<DeviceModel> modelIt = modelMap.keySet().iterator();
		int i=0;
		while(modelIt.hasNext()) {
			DeviceModel rmodel = modelIt.next();
			// 得到该子设备的类型
			PSRDef psrDef = new PSRDef();
			String psrType = rmodel.getClassDef().getClassId();
			String psrName = rmodel.getClassDef().getClassAlias();
			SubClassDef[] SubClassDef = rmodel.getSubClassDef();
			...
			subRelateField = modelMap.get(rmodel);
			exp = builder.equal(builder.getRoot().get(subRelateField, String.class),
					OId);
			...
			Iterator<FieldDetail> fieldIt = rmodel.getFieldDef()
					.details();
			...
			try {
				...
				set = rmodel.search(queryReturnField, exp,null);
			} catch (Exception e) {
				logger.info("数据库查询出错");
				resp.setCode(ReturnCode.SQLERROR);
				return resp;
...
//						}
					...
					fields.setFieldAlias(model.getFieldDef()
							.find(queryReturnField[k]).getFieldAlias());
