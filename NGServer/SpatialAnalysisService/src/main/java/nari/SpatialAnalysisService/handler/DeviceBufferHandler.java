...
public class DeviceBufferHandler {
	public DeviceBufferResponse deviceBuffer(DeviceBufferRequest request) {
		...
		Expression exp0 = builder.equal(
				builder.getRoot().get("OID", String.class), oid);
		ResultSet set = ResultSet.NONE;
		try {
			set = model.search(null, exp0,null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		Device dev = set.getSingle();
		// 得到缓冲区
		double degree = request.getRadius();
		Geometry buffer = dev.asSpatialDevice().buffer(degree);
		...
		for (int i = 0; i < condLength; i++) {
			String condClassId = cond[i].getPsrType();
			try {
				model = ms.fromClass(condClassId, true);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			// 条件组合
			...
			Link link = cond[i].getLink();cond[i].getLink();
			// 得到条件
				...
				if (true) {
					...
					if(("MULTIPOLYLINE").equalsIgnoreCase(geometryType)){
						...
						for(int k=0;k<lineNum-1;k++){
							Geometry polyLine = multipolyLine.getGeometry(k);
							startDouble[k+1] = polyLine.getCoordinates().length+startDouble[k];
						}
