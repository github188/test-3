...
public class GeometryReader {
	...
	private static final AtomicReference<GeometryReader> reader = new AtomicReference<GeometryReader>();
	
	public static GeometryReader getReader(){
		if(reader.get()==null){
			GeometryReader gr = new GeometryReader();
			reader.compareAndSet(null, gr);
		}
		return reader.get();
	}
	
	public Geometry read(Object shape){
		JGeometry geom = readGeometry(shape);
		Geometry g = buildGeometry(geom);
		return g;
	}
	...
	private Geometry buildGeometry(JGeometry geom) {
	...
	private List<Coordinate> getHull(Coordinate c1, Coordinate c2, Coordinate c3) {
		List<Coordinate> hull = new ArrayList<Coordinate>();
	...
	private Coordinate computeCoordinate(Coordinate[] coords, Coordinate coord) {
		double dis = 100.0D;
		Coordinate tmp = null;
	...
	private Coordinate[] getCircle(Coordinate c1, Coordinate c2, Coordinate c3) {
